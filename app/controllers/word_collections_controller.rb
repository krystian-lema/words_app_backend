class WordCollectionsController < ApplicationController
  ...
  before_action :check_permission, only: [:change_collection_name, :delete_collection]
  ...
  def find_collection
    @collection = if WordCollection.where(id: params[:id]).blank?
                    nil
                  else
                    WordCollection.find(params[:id])
                  end
  end
